...
func (r *Renderer) loadShared() error {
	...
	r.mu.Lock()
	r.shared = t
	r.pageCache = map[string]*template.Template{} // reset cache on reload
	r.mu.Unlock()
	return nil
}
...
func (r *Renderer) MountGet(route string, page string, dataFn ViewDataFunc) *Renderer {
	...
	r.routeBuilder.Get(normalized, h)
...
func (r *Renderer) wrap(next http.Handler) http.Handler {
	for i := len(r.routeBuilder.middleware) - 1; i >= 0; i-- {
		next = r.routeBuilder.middleware[i](next)
	}
	return next
}
